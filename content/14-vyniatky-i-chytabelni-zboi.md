...
:::trace Виняток змінює звичайний маршрут виконання
before: `text = "п’ять"`; змінної `number` ще немає.
step: Спроба | `number = int(text)` | `int()` не може перетворити цей текст і піднімає `ValueError`.
step: Перехід | `except ValueError:` | Python пропускає решту блоку `try` і шукає відповідний обробник.
step: Корисне повідомлення | `print("Потрібне ціле число.")` | Обробник пояснює очікуваний формат без технічного traceback для користувача.
after: Програма завершується контрольовано; `number` не створено, надруковано корисне повідомлення.
meaning: `try` не робить операцію безпомилковою — він описує окремий маршрут для конкретного передбачуваного збою.
:::
...
:::parsons Склади повний маршрут `try` / `except` / `else`
...
:::recall
Закрий код і поясни ролі `try`, `except`, `else` та `finally`. Потім назви, чому `except Exception:` зазвичай гірший за конкретний тип винятку.
:::
