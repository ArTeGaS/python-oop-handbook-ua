...
Останній рядок traceback приблизно такий:
...
Він може перехопити навіть сигнали завершення, приховує причину й залишає невідомий стан.
...
`calculate()` теж може мати `ValueError` через дефект, який тут буде неправильно пояснений введенням. Звузь `try` до перетворення, а успішну роботу перенеси в `else`.
...
Програма продовжує роботу з невідомим станом, а traceback втрачено. Або оброби конкретну очікувану ситуацію, або дай винятку піднятися, або журналюй його з повним контекстом на зовнішній межі.
