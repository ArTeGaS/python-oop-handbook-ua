...
:::trace Один тест як контрольована зміна стану
before: `robot.energy = 10`, `robot.distance = 0`.
step: Підготовка | `robot = Robot(10)` | Створюється відомий початковий стан.
step: Дія | `robot.move(3)` | Метод віднімає `3` від енергії та додає `3` до відстані.
step: Перша перевірка | `assert robot.energy == 7` | Якщо енергія інша, виконання зупиниться з `AssertionError`.
step: Друга перевірка | `assert robot.distance == 3` | Перевіряється друга видима частина поведінки.
step: Сигнал успіху | `print("Базові твердження виконано.")` | Рядок виконується лише після двох успішних тверджень.
after: `robot.energy = 7`, `robot.distance = 3`; обидві перевірки пройдено.
meaning: Тест фіксує початковий стан, одну дію та спостережуваний результат; він не повинен залежати від порядку запуску інших тестів.
:::
...
:::parsons Склади перевірку за схемою AAA
