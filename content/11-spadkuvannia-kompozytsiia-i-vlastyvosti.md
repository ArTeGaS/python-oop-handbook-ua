...
:::trace Як дочірній клас знаходить успадкований метод
before: Клас `Bicycle` не має власного `__init__()` або `move_text()`.
step: Зв’язок класів | `class Bicycle(Vehicle):` | `Vehicle` записаний як батьківський клас.
step: Створення | `Bicycle("Ластівка", 18)` | Python не знаходить `__init__` у `Bicycle` і використовує `Vehicle.__init__`.
step: Збереження стану | `self.name`, `self.speed` | У новому об’єкті з’являються атрибути, описані батьківським класом.
step: Пошук поведінки | `bicycle.move_text()` | Python не знаходить метод у `Bicycle` і бере `Vehicle.move_text`.
after: Об’єкт `Bicycle` має стан `Ластівка`, `18` і повертає успадкований текст руху.
meaning: Успадкування дозволяє спеціалізованому класу повторно використати спільний стан і поведінку батьківського класу.
:::
...
:::completion Обери зв’язок за змістом
Заміни пропуски одним із двох підходів: успадкуванням або композицією.

- `ElectricBicycle` ______ `Bicycle`, бо електровелосипед є різновидом велосипеда.
- `Bicycle` ______ об’єкт `Battery`, бо велосипед має батарею всередині.
