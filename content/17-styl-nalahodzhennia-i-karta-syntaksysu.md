...
:::trace Спостереження локалізує логічний дефект
before: `balance = 100`, `payment = 30`; очікувана поведінка — зменшення балансу до `70`.
step: Фактична операція | `balance += payment` | Оператор додає `30`, хоча предметна дія «платіж» вимагала віднімання.
step: Фактичний стан | `balance == 130` | Програма працює без винятку, але стан суперечить очікуванню.
step: Порівняння | `Очікували 70, отримали 130` | Два конкретні числа звужують пошук до операції зміни балансу.
step: Одна правка | `balance -= payment` | Змінюється одна причина, а не весь фрагмент одразу.
step: Повторна перевірка | `balance == 70` | Той самий сценарій доводить, що поведінка відновлена.
after: Після виправлення баланс дорівнює `70`; очікування і фактичний стан збігаються.
meaning: Логічний дефект знаходять через явне очікування, спостереження стану та одну контрольовану зміну, а не випадкові редагування.
:::
...
:::parsons Віднови дисципліну налагодження
