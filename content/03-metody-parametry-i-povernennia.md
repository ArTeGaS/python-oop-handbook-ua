...
:::trace Аргумент стає значенням параметра
before: `hero.health = 70`; ім’я `amount` ще не має значення для цього виклику.
step: Місце виклику | `hero.heal(15)` | `15` — аргумент: конкретне значення, яке передає цей виклик.
step: Загальний опис | `def heal(self, amount):` | `amount` — параметр: ім’я, під яким метод прийматиме різні значення.
step: Зіставлення | `amount = 15` | Під час цього виклику Python пов’язує параметр `amount` з аргументом `15`.
step: Зміна стану | `self.health += amount` | Для цього об’єкта виконується `70 + 15`.
after: `hero.health = 85`; після завершення методу локальне ім’я `amount` більше не потрібне.
meaning: Аргумент живе у конкретному виклику, параметр — у загальному описі методу; під час виклику Python тимчасово з’єднує їх.
:::
...
:::completion Розділи два місця навмисно
