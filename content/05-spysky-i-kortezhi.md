...
:::trace Як індекс обирає елемент
before: `items = ["ліхтар", "карта", "ключ"]`; список має три позиції.
step: Перша позиція | `items[0]` | Відлік зліва починається з нуля, тому результат — `"ліхтар"`.
step: Друга позиція | `items[1]` | Індекс `1` обирає `"карта"`.
step: Остання позиція | `items[-1]` | Від’ємний індекс рахує справа, тому результат — `"ключ"`.
after: Надруковано три значення; порядок і вміст `items` не змінилися.
meaning: Індекс указує на позицію у впорядкованій колекції; читання позиції не є зміною списку.
:::
...
:::completion Доповни три різні операції
