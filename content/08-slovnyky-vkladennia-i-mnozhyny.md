...
:::trace Метод змінює одне значення за ключем
before: `self.settings["volume"] == 70`.
step: Виклик | `profile.change_setting("volume", 40)` | Аргументи передають ключ `"volume"` і нове значення `40`.
step: Захисна перевірка | `key not in self.settings` | Метод спершу переконується, що такий ключ дозволений.
step: Вибір комірки | `self.settings[key]` | Значення `key` дорівнює `"volume"`, тому обрано саме налаштування гучності.
step: Заміна | `self.settings[key] = value` | Старе значення `70` замінюється на `40`.
after: `self.settings["volume"] == 40`; інші налаштування лишилися без змін.
meaning: Ключ називає потрібну комірку словника, а метод обмежує й пояснює допустиму зміну стану об’єкта.
:::
...
:::completion Доповни безпечне оновлення статистики
Заповни пропуски так, щоб перший виклик для нового ключа починав відлік з нуля.

```python fragment
def add_stat(self, key, amount=1):
    current = self.statistics.____(key, ____)
    self.statistics[____] = current + amount
```
