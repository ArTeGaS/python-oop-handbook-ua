...
:::trace Як випадковість стає керованою
before: Є два окремі `Random(7)`; жодна місія ще не створена.
step: Передавання залежності | `MissionGenerator(places, random.Random(7))` | Кожен генератор отримує власне джерело випадковості з однаковим початковим станом.
step: Вибір місць | `random_source.sample(..., k=2)` | Однаковий стан джерел дає однакову першу пару місць.
step: Вибір складності | `random_source.randint(1, 5)` | Наступне однакове рішення дає однакову складність.
step: Створення | `return Mission(...)` | Обидва генератори збирають рівні за даними місії.
step: Перевірка | `first_description == second_description` | Порівняння двох описів повертає `True`.
after: Створено дві окремі, але однакові за даними перші місії; результат перевірки — `True`.
meaning: Зерно не прибирає випадковий алгоритм, а дає відтворювану послідовність; передана залежність робить це локальним і тестованим.
:::
...
:::completion Залиш випадковість замінною
