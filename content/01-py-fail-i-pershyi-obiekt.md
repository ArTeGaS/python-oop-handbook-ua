...
:::practice Доведи, що розширення не запускає код
...
:::trace Від опису класу до першої дії
before: Об’єкта `robot` ще немає, термінал порожній.
step: Опис | `class Robot:` | Python запам’ятовує, які дії матимуть роботи; тіло методу ще не виконується.
step: Створення | `robot = Robot()` | З’являється конкретний об’єкт, на який посилається ім’я `robot`.
step: Виклик | `robot.say_hello()` | Крапка обирає метод об’єкта, а Python передає цей об’єкт у `self`.
step: Видимий результат | `print("Привіт! Я Робі.")` | Команда всередині методу записує один рядок у термінал.
after: Об’єкт `robot` існує, у терміналі є `Привіт! Я Робі.`
meaning: Клас описує можливість, об’єкт є конкретним учасником, а виклик методу запускає дію.
:::
...
![VS Code з відкритим robot.py; жовтою рамкою позначено кнопку Run Python File](assets/images/vscode-run-python-file.png)
...
:::parsons Поверни програму в робочий порядок
