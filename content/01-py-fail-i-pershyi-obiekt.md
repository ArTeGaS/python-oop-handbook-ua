...
У цьому розділі ми не встановлюємо редактор і не налаштовуємо Python. Вважаємо, що папка вже відкрита у VS Code, а команда запуску працює. Наше завдання — зрозуміти, **що саме ми запускаємо**, і відразу створити об’єкт, який щось робить.
...
Створи файл `message.txt` із таким вмістом:

```python fragment file=message.txt
print("Цей текст виконав Python.")
```

Якщо явно передати цей файл інтерпретатору, він спробує виконати вміст навіть без розширення `.py`:

```powershell
python message.txt
```

На macOS або Linux команда зазвичай така:
...
Запусти файл кнопкою **Run Python File** у правому верхньому куті VS Code. Редактор збереже файл, відкриє вбудований термінал і передасть `robot.py` вибраному Python.
...
Приберемо двокрапку після назви класу:

```python error file=robot_broken.py
class Robot
    def say_hello(self):
        print("Привіт!")
```

Python не може завершити читання першого рядка й показує `SyntaxError`. Останній рядок traceback — повідомлення про тип проблеми. Трохи вище Python показує файл, номер рядка й місце, біля якого перестав розуміти код.
...
Інший частий збій — відсутній відступ:
...
Після `class Robot:` інтерпретатор очікує вкладений блок. Тому повідомлення буде пов’язане з відступом. Виправляй не весь файл навмання, а перший рядок свого коду, на який вказує traceback.
