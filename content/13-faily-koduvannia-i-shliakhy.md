...
:::trace Об’єкт шляху описує адресу частинами
before: Є лише два текстові фрагменти: папка `notes` та ім’я `lesson.txt`.
step: Побудова | `Path("notes") / "lesson.txt"` | `Path` складає платформонезалежний об’єкт шляху; файл ще не відкривається.
step: Батьківська папка | `path.parent` | Результат описує `notes`.
step: Ім’я | `path.name` | Результат описує останню частину `lesson.txt`.
step: Суфікс | `path.suffix` | Результат описує розширення `.txt`.
after: Об’єкт `path` описує адресу; на диску нічого не створено й не змінено.
meaning: `Path` спершу дає безпечний опис місця, а окрема операція читання або запису вже працює з файловою системою.
:::
...
:::parsons Віднови безпечний порядок роботи з файлом
