...
:::trace Введений текст стає числом
before: Користувач набрав символи `12`; змінних `answer` і `number` ще немає.
step: Читання | `answer = input(...)` | `input()` завжди повертає рядок, тому `answer` отримує `"12"`.
step: Перевірка типу | `type(answer).__name__` | Термінал показує `str`.
step: Перетворення | `number = int(answer)` | `int()` створює ціле число `12`; початковий рядок не змінюється.
step: Обчислення | `number + 1` | Числова операція дає `13`.
after: `answer == "12"`, `number == 12`, надруковано `Наступне число: 13`.
meaning: Текстове введення треба явно перевірити або перетворити перед числовими обчисленнями.
:::
...
:::completion Дай циклу шлях до завершення
